...
class Test {
    ...
    public static void main(String[] args) {
        ...
        request.setType(PreviewEmailTemplateDto.type.WELCOME_EMAIL);
        request.setContent("xxx");
        request.setSubject("欢迎加入 {{app_name}}");
        request.setSender("{{client_name}}");
        request.setExpiresIn(300);
        request.setTplEngine(PreviewEmailTemplateDto.tplEngine.HANDLEBAR);
        
        PreviewEmailTemplateRespDto response = managementClient.previewEmailTemplate(request);
